...
class BladeInstance implements BladeInterface
{
	...
	/**
	 * @var Filesystem $cache_file_system The default path for cached php.
	 */
	private $cache_file_system;
	 ...
	 * @param Filesystem $cache_file_system The default path for cached php
	 */
	public function __construct(string $path, Filesystem $cache_file_system, $cache_path, DirectivesInterface $directives = null)
	{
		$this->path = $path;
		$this->cache_file_system = $cache_file_system;
	...
	private function getViewFactory(): FactoryInterface
	{
		...
		$resolver->register("blade", function () {
			...
			//ローカルファイルシステムのとき
			if($this->cache_file_system instanceof LocalFilesystem) {
				if (!is_dir($this->cache_path)) {
					$this->cache_file_system->makeDirectory($this->cache_path, 0777, true);
				}
			}
			
			//コンパイルする
			$blade = new BladeCompiler($this->cache_file_system, $this->cache_path);
